...
def explode_genres(df: pd.DataFrame) -> pd.DataFrame:
    """Explode Genres column to create one row per genre per app."""
    df_exploded = df.copy()
...
def plot_family_game_genres_comparison(
    ...
    """Plot bar charts for FAMILY and GAME categories."""
    if root is None:
...
def main(
    ...
    """Main function to run genre analysis."""
    if root is None:
        root = Path(__file__).parent.parent
    else:
        root = Path(root)
    ...
    analyze_genre_focus_recommendations(stats_df, root=root)
